...
double main(void) 
{
	...
	char username[4][14];
	strcpy(username[0], "0000");
	strcpy(username[1], "admin");
	strcpy(username[2], "user");

	char password[4][14];  //define database default username and password
	strcpy(password[0], "1111");
	strcpy(password[1], "admin");
	strcpy(password[2], "user");
	...
	while (1) //define while loop equals to true until break statement
	{
		...
		{
			if (strcmp(username[x], inputID) == 0 && strcmp(password[x], inputPassword) == 0) //comparing inputID variable and username variable string  
			{
				printf("\n****Access Granted****\n"); // print access granted if above if condition meets 
				strcpy(currentUser, username[x]);
				returnCase = 1; //assigning returnCase = 1 to mark user input as true
	...
	while (1) //define while loop equals to true until break statement
	{
		...
		while (1)	
		{
			...
			if (currentLocal > -1 && currentLocal < INT_MAX)
			{
				break;
			}
			printf("\nInvalid Input!\n");
		...
		while (1)
		{
			...
			rewind(stdin);
			if(currentImport > -1 && currentImport < INT_MAX)
			{
				break;
			}
			printf("\nInvalid Input!\n");
		...
		while (1)
		{
			...
			rewind(stdin);
			if (currentTotalDeaths > -1 && currentTotalDeaths < INT_MAX)
			{
				break;
			}
			printf("\nInvalid Input!\n");
		...
		while (1)
		{
			...
			rewind(stdin);
			if (currentTotalRecovered > -1 && currentTotalRecovered < INT_MAX)
			{
				break;
			}
			printf("\nInvalid Input!\n");
		...
		while (1)
		{
			...
			if (userDigitPrompt == 1 || userDigitPrompt == 2)
			{
				break;
			}
			else
		...
		while (1)
		{
			...
			rewind(stdin);
			if (userYesNoPrompt == 'y' || userYesNoPrompt == 'Y' || userYesNoPrompt == 'n' || userYesNoPrompt == 'N')
			{
				break;
			}
			else
		...
		if (userYesNoPrompt == 'n' || userYesNoPrompt == 'N') { // if user input equals to 'n' returns true
		...
		while (1)  //define while loop equals to true until break statement
		{
			yesterdayTable(currentNewCases, currentTotalCases, currentTotalDeaths, currentTotalRecovered, currentUnderTreatment, ratesCurrentTotalDeaths, ratesCurrentTotalRecovered, ratesCurrentUnderTreatment, dayYesterday);
			currentLocal = -1, currentImport = -1, currentTotalDeaths = -1, currentTotalRecovered = -1;
			jam();
			printf("\nDay %3.0lf", dayToday); //prints out today's day no.
			printf("\n========\n");  // printing out equal sign as dividing line
			printf("Key in Today's data:");  // prompting users to key in today's data
	...
	int session = difftime(end, start);
	int seconds;                              // convert value in seconds to minutes and seconds
	int minutes;
	if (session > 60)
	{
		minutes = session / 60;
	...
	printf("\n\n\nUser ID --- %s 's Total clock-in time :  %d minutes and %d seconds\n\n", currentUser, minutes, seconds); //outputs total user's total usage time
	printf("\n\n\nM A L A Y S I A  S T A N D A R D  T I M E (GMT+8). > ");
	time_t rawtime;
	struct tm* info;
	time(&rawtime);
	info = localtime(&rawtime);
...
User ID --- 0000 's Total clock-in time :  1 minutes and 12 seconds
